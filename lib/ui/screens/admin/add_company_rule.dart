...
class AddCompanyRule extends StatefulWidget {
  AddCompanyRule({this.companyId});
  String companyId;
  @override
  State createState() => _AddCompanyRuleState();
}

class _AddCompanyRuleState extends State<AddCompanyRule> {
  ...
  Future _addData({CompanyRule rule}) async {
    if (_formKey.currentState.validate()) {
      try {
        SystemChannels.textInput.invokeMethod('TextInput.hide');
        await _changeLoadingVisible();

        CompanyAdminRepo.addCompanyRule(rule).then((onValue) {
          if (onValue) {
            AlertDiag.showAlertDialog(
                context,
                'Status',
                'Company Rule Successfully added',
                RouteConstants.ADMIN_VIEW_COMPANY_RULES);
          } else {
            AlertDiag.showAlertDialog(
                context,
                'Status',
      ...
      child: RaisedButton(
          ...
          onPressed: () {
            setState(() {
              ...
              rule.title = ruleController.text;
              rule.rule = descriptionController.text;
